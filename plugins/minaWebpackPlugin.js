...
function _inflateEntries(entries = [], dirname, entry) {
  ...
  usingComponents &&
    Object.values(usingComponents).forEach(item => inflateEntries(entries, dirname, item))
  subpackages &&
    subpackages.forEach(subpackage =>
      subpackage.pages.forEach(item =>
        inflateEntries(entries, dirname + `/${subpackage.root}`, item)
      )
    )
}
...
function inflateEntries(entries, dirname, entry) {
  ...
  if (typeof entry !== 'string') {
    throw new Error(
      '入口文件位置获取有误，请检查webpack版本或webpack配置(webpack.config.js)是否发生变化'
    )
  }
...
class MinaWebpackPlugin {
  ...
  applyEntry(compiler, done) {
    ...
    this.entries
      ...
      .forEach(item =>
        new SingleEntryPlugin(context, './' + item, replaceExt(item, '')).apply(compiler)
      )
